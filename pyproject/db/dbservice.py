...
def get_sync_db():
    global g_sync_db
    if None == g_sync_db:
        g_sync_db = ffext.ffdb_create('mysql://localhost:3306/root/root/pcgame')
    return g_sync_db

def get_async_db():
    global g_async_db
    if None == g_async_db:
        g_async_db = ffext.ffdb_create('mysql://localhost:3306/root/root/pcgame')
    return g_async_db
...
def load_player(player, callback):
    sql = "SELECT * FROM `%s`  WHERE `ID` = '%d' " % (format_player_table(player), player.id())
    def cb(ret):
